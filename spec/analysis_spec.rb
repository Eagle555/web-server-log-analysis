  ...
  describe '#load_file and process' do
    ...
    describe 'number of visits' do
      it 'process the log and expect number of visits' do
        expect(@parser.number_of_visits).to eq([['/about/2', 74], ['/contact', 73], ['/index', 68], ['/about', 64],
                                                ['/help_page/1', 61], ['/home', 60]])
      end
    end

    describe 'number of unique visits' do
      it 'process the log and expect number of visits' do
        expect(@parser.number_of_unique_visits).to eq([['/help_page/1', 7], ['/about', 5], ['/contact', 4],
                                                       ['/home', 3], ['/about/2', 3], ['/index', 1]])
      end
    end
  end
end
